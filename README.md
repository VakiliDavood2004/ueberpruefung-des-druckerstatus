...
# 🖨️ Überprüfung des Druckerstatus  
**بررسی وضعیت پرینترها با رابط گرافیکی چندزبانه**
...
| کتابخانه گرافیکی | PyQt5 |
| نوع رابط | گرافیکی، چندزبانه |
| زبان‌های پشتیبانی‌شده | 🇮🇷 فارسی، 🇩🇪 آلمانی، 🇬🇧 انگلیسی |
| فرمت ورودی داده | فایل CSV استاندارد |
| نوع بررسی | دستی و خودکار (هر ۲ ثانیه) |
| هشدار | آلارم صوتی هنگام خرابی پرینتر |
| طراحی رابط | CSS سفارشی (Qt StyleSheet) |
...
| `assets/alert.wav` | فایل صوتی هشدار |
...
| ستون‌ها | توضیح |
|--------|-------|
| PrinterName | نام پرینتر |
| Paper | وضعیت کاغذ |
| Ink | وضعیت جوهر |
| Toner | وضعیت تونر |
| Board | وضعیت برد الکترونیکی |
...
| مرحله | دستور یا توضیح |
|--------|----------------|
| نصب پیش‌نیاز | `pip install PyQt5` |
| اجرای برنامه | `python main.py` |
| مراحل استفاده | انتخاب زبان → بارگذاری فایل CSV → مشاهده وضعیت → بررسی خودکار → دریافت گزارش |
...
| وضعیت | پرینتر | ایراد |
|--------|---------|--------|
| ✅ سالم | Brother HL-L2350 | — |
| ❌ خراب | Canon Pixma G3000 | جوهر |
| ❌ خراب | HP LaserJet 1020 | تونر |
| ❌ خراب | Epson L3150 | کاغذ |
...
| نکته | توضیح |
|------|--------|
| محل فایل صوتی | `alert.wav` باید در پوشه `assets/` یا کنار `main.py` باشد |
| کدگذاری فایل CSV | UTF-8 بدون BOM |
| خطاهای احتمالی | بررسی تنظیمات encoding در صورت بروز خطاهای "codec" یا "charmap" |
| فایل تست | استفاده از `sample.csv` در پوشه `data/` برای تست سریع |
...
| نام | تخصص | هدف پروژه |
|-----|-------|------------|
| داوود وکیلی | طراحی رابط گرافیکی، آموزش PyQt5، ساخت ابزارهای مانیتورینگ | ارائه تجربه‌ای حرفه‌ای، آموزشی و کاربردی برای توسعه‌دهندگان علاقه‌مند به پروژه‌های چندزبانه و گرافیکی |
